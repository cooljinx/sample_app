...
class User < ActiveRecord::Base
  ...
  validates :password,  :presence => true,
                                    :confirmation => true,  # automatically creates "password_confirmation"  virtual attribute
                                    :length => { :within => 6..40 }
  ...
  def self.authenticate_with_salt(id, cookie_salt)
      user = find_by_id(id)
      (user && user.salt == cookie_salt) ? user : nil   # using ternary operator to compress if-else combination
